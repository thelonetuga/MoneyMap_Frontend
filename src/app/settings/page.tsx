...
interface Category {
  id: number;
  name: string;
  subcategories: SubCategory[]; // Backend devolve isto aninhado
}
...
export default function SettingsPage() {
  ...
  const handleDeleteAccount = async (id: number) => {
    if (
      !confirm(
        "Tem a certeza? Apagar uma conta apaga todas as transações dela!"
      )
    )
      return;
    try {
      ...
      alert("Erro ao apagar conta (verifique se tem permissão) ❌");
    }
  };
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // O backend devolve a categoria criada (subcategories virá vazio ou null)
      const newCat = { ...res.data, subcategories: [] };
  ...
  const handleDeleteSubCategory = async (subId: number, catId: number) => {
    ...
    try {
      ...
      alert("Erro ao apagar (pode estar em uso numa transação).");
    }
  };
  ...
  const inputClass =
    "w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-medium";

  if (loading)
    return (
      <div className="p-10 text-center text-gray-400">
        A carregar definições... ⚙️
      </div>
    );
      ...
      {activeTab === "accounts" && (
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            {accounts.length === 0 && (
              <p className="text-gray-400 italic">Nenhuma conta criada.</p>
            )}
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-gray-50 p-4 border-t border-gray-100 animate-in slide-in-from-top-2 duration-200">
                    <div className="flex flex-wrap gap-2 mb-4">
                      {cat.subcategories?.map((sub) => (
                        <div
                          key={sub.id}
                          className="flex items-center gap-1 px-3 py-1 bg-white border border-gray-200 rounded-lg text-sm text-gray-600 group hover:border-red-200 transition-colors"
                        >
                          <span>{sub.name}</span>
                          {/* Botão X que só aparece quando passas o rato (group-hover) ou sempre visível em mobile */}
                          <button
                            onClick={(e) => {
                              e.stopPropagation(); // Impede que o acordeão feche ao clicar no X
                              handleDeleteSubCategory(sub.id, cat.id);
                            }}
                            className="ml-1 text-gray-300 hover:text-red-500 font-bold px-1 rounded-full"
                            title="Apagar"
                          >
                            ×
                          </button>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 h-fit sticky top-24">
            ...
            <p className="mt-4 text-sm text-gray-500">
              Crie categorias gerais aqui (ex: Alimentação, Transportes) e
              adicione as específicas (ex: Restaurantes, Uber) clicando na lista
              à esquerda.
            </p>
          </div>
        </div>
