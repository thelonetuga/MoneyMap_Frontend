      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-1">
            <Link href="/" className={linkStyle('/')}>Dashboard</Link>
            <Link href="/transactions" className={linkStyle('/transactions')}>Histórico</Link>
            <Link href="/settings" className={linkStyle('/settings')}>Definições ⚙️</Link>
            
            {/* LINK PREMIUM: SÓ APARECE SE TIVERES O ROLE CERTO */}
            {(user?.role === 'premium' || user?.role === 'admin') && (
                <Link href="/imports" className="text-amber-600 font-bold px-4 py-2 text-sm">
                  Importar ⭐
                </Link>
            )}
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-200 bg-gray-100 relative">
                  {user?.profile?.avatar_url ? (
                      <img src={`http://127.0.0.1:8000${user.profile.avatar_url}`} alt="Avatar" className="w-full h-full object-cover" />
                  ) : (
              ...
              <button onClick={logout} className="p-2 text-gray-400 hover:text-red-500 transition-colors rounded-full hover:bg-red-50" title="Sair">
